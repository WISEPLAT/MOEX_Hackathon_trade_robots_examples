...
@dataclass
class OBstatExtractor(BaseExtractor):
    state: BaseState
    ...
    def extract(
        ...
        while _block_size < block_size:
            ...
            for obstat in obstats:
                i += 1
                if obstat.ts < down_limit:
                    logger.info(f'Already extract. Begin: {obstat.ts} | {key}::{down_limit}')
                    continue
