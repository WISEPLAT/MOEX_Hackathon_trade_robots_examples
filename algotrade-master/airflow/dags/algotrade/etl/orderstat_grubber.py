...
def run():
    ...
    extractor = OrderstatExtractor(state=state)
    orderstat_transformer = OrderstatTransformer()

    for ticker in tickers:
        logger.info(f'Starting {ticker.secid}')

        while state.get(f'status::orderstat::{ticker.secid}', 0) == 0 and state.get(f'extracting::orderstat::{ticker.secid}', 0) == 0:
            raw_data = extractor.extract(ticker=ticker, block_size=batch_size)

            data = orderstat_transformer.transform(data=raw_data, ticker=ticker.secid)
            ...
            loader.load(data=data, table="orderstats", key=f'orderstat::{ticker.secid}')
