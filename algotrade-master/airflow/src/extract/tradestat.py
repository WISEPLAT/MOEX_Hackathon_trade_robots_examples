...
@dataclass
class TradestatExtractor(BaseExtractor):
    state: BaseState
    ...
    def extract(
        ...
        while _block_size < block_size:
            ...
            tradestats = ticker_ts.tradestats(
            ...
            for tradestat in tradestats:
                i += 1
                if tradestat.ts < down_limit:
                    logger.info(f'Already extract. TS: {tradestat.ts} | {key}::{down_limit}')
                    continue
