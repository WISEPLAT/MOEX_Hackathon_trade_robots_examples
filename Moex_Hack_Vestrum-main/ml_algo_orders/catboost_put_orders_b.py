...
X = data.drop(['put_orders_b', 'secid'], axis=1)
y = data['put_orders_b']
...
model = CatBoostRegressor(iterations=1000, learning_rate=0.1, depth=10, verbose=False)
model.fit(X_train_scaled, y_train, eval_set=(X_test_scaled, y_test), use_best_model=True)

# Сохранение модели
model.save_model('final_catboost_model_put_orders_b.cbm')
